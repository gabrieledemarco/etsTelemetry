...
class Login:
    def __init__(self, Dbs: DbService):
        self.usernames = Dbs.get_all_value_in_column(name_column="nickname",
                                                     name_table="users")
        self.password = Dbs.get_all_value_in_column(name_column="pass_word",
                                                    name_table="users")

    def LogIn_Authentication(self):
        hashed_passwords = hasher(self.password).generate()
        authenticator = authenticate(self.usernames, hashed_passwords,
...
class authenticate:
    def __init__(self, usernames, passwords, cookie_name, key, cookie_expiry_days=30):
                ...
                if login_form.form_submit_button('Login'):
                    ...
                    if self.index != None:
                        try:
                            ...
                            raise ValueError(
                                "Please enter hashed passwords and not plain text passwords into the 'authenticate' module.")
                    else:
                        st.session_state['authentication_status'] = False
